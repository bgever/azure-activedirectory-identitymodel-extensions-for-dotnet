param(
    ...
    [string]$runTests="YES",
    [string]$updateCoreFxVersion="YES")
...
$netCoreAppOldVersion = $buildConfiguration.SelectSingleNode("root/netCoreAppOldVersion").InnerText;
$netCoreAppNewVersion = $buildConfiguration.SelectSingleNode("root/netCoreAppNewVersion").InnerText;
...
function SetCoreFxVersion([string]$fileName, [string]$oldVersion, [string]$newVersion, [string]$netCoreAppOldVersion, [string]$netCoreAppNewVersion)
{
    $content = Get-Content -Path $fileName -raw;
    if ((-not $content.Contains($newVersion)) -and ($content.Contains("-rc3-")))
    {
        Write-Host ">>> SetCoreFxVersion: " $fileName ", " $oldVersion ", " $newVersion ", " $netCoreAppOldVersion ", " $netCoreAppNewVersion
        $newContent = $content -replace $netCoreAppOldVersion, $netCoreAppNewVersion;
        $newContent2 = $newContent -replace $oldVersion, $newVersion;
        Set-Content $fileName $newContent2;
    }
}
...
if ($updateCoreFxVersion -eq "YES")
{
    ...
    foreach($project in $projects) {
        $name = $project.name;
        SetCoreFxVersion "$root\src\$name\project.json" $coreFxOldVersion $coreFxNewVersion $netCoreAppOldVersion $netCoreAppNewVersion;
    }

    $testProjects = $buildConfiguration.SelectNodes("root/projects/test/project")
    foreach ($testProject in $testProjects) {
        $name = $testProject.name;
        SetCoreFxVersion "$root\test\$name\project.json" $coreFxOldVersion $coreFxNewVersion $netCoreAppOldVersion $netCoreAppNewVersion;
    }
}
